        ...
        fetch(file)
            ...
            .then(data => {
                ...
                for (let i = 0; i < faculties.length; i++) {
                    const header = faculties[i].getElementsByClassName('header')[0];
                    const h3 = header.getElementsByTagName('h3')[0];
                    const title = faculties[i].getElementsByClassName('Titulli')[0];
                    const titleH2 = title ? title.getElementsByTagName('h2')[0] : null;
                    const txtValue = h3.textContent || h3.innerText;
                    const titleValue = titleH2 ? titleH2.textContent || titleH2.innerText : '';

                    if (txtValue.toLowerCase().indexOf(filter) > -1 || titleValue.toLowerCase().indexOf(filter) > -1) {
...
function resetPage() {
    fetch('indexPrivate.html')
        ...
        .then(data => {
            const parser = new DOMParser();
            const doc = parser.parse (data, 'text/html');
